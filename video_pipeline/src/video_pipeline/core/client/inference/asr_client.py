...
class QwenASRClient:
    def __init__(self, client_url: str, config: QwenASRConfig):
        ...
        self.client = AsyncOpenAI(base_url=self.client_url, api_key=config.api_key)
    ...
    async def check_server_health(self) -> bool:
        try:
            ...
            if self.model_name not in available_models:
                logger.error(
                    f"ASR model '{self.model_name}' not found. Available: {available_models}"
                )
                return False
    ...
    async def _transcribe_single(self, audio_path: str) -> str:
        with open(audio_path, "rb") as f:
            response = await self.client.audio.transcriptions.create(
                model=self.model_name, file=f, response_format="text"
            )
