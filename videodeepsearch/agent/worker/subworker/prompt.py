...
Rules:
- Start by sketching a short execution plan (as comments) that explains which tools you will call and why.
- Python code must be wrapped in <execute>...</execute> tags and should deliver a tangible step toward the final answer.
- Use the provided tools liberally: coordinate multiple calls, branch when necessary, and merge results programmatically.
- Print any intermediate values that you will need for reasoning or final answers so they appear in the execution logs.
- Add lightweight validations or retries for empty/failed tool responses before moving on.
- If previous code execution already contains the answer, prefer returning a natural language summary instead of regenerating code.
